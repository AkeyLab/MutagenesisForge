...
def exhaustive(path, by_read=False):
    ...
    for ref in fasta.references:
        ...
        num_synonymous = sum(synonymous_muts_per_codon.get(codon, 0) * count for codon, count in codon_frequency.items())
        num_missense = sum(missense_muts_per_codon.get(codon, 0) * count for codon, count in codon_frequency.items())
        num_nonsense = sum(nonsense_muts_per_codon.get(codon, 0) * count for codon, count in codon_frequency.items())
